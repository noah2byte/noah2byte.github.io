...
> 작성일: 2026-07-16 · 환경: Windows 11 (네이티브), Hermes Agent v0.18.2 (2026.7.7.2), Ollama(127.0.0.1:11434)<br>
목적: **API 키·구독 없이, 비용이 전혀 발생하지 않는 구성**으로 Hermes Agent를 설치·세팅하고 검증할 때 참조할 수 있는 실전 기록. 실제 진행 과정에서 겪은 실패와 그 원인, 그리고 무료 구성의 구조적 제약을 함께 정리한다.<br>
...
이 표가 가리키는 결론은 명확하다. **이 노트북에서는 "Hermes의 최소 요구사항(64K)을 만족하면서 동시에 에이전틱 추론이 가능한" 로컬 모델이 존재하지 않는다.** 컨텍스트가 넉넉한 gemma4:e2b는 추론력이 부족하고, 추론력에 여지가 있는 qwen3 계열은 이 하드웨어에서 컨텍스트 요구를 만족하는 크기까지 못 올라간다 — *이 문단은 최초 판단이었고, 바로 아래에서 뒤집힌다.*

### 4.3 반전: qwen3:8b의 관문③ 재시도 — "능력 없음"이 아니라 "설정값 문제"였다

4.2절까지는 qwen3:8b가 관문②(Hermes 최소 컨텍스트)에서 막혀 관문③(격리 테스트)을 시도조차 못 했다. 그런데 Hermes를 거치지 않고 4.1절과 동일한 방식으로 **Ollama API에 직접** qwen3:8b를 격리 테스트해보니 결과가 달랐다:
...
**실행 결과: "The weather in Seoul is currently 31°C, with sunny skies and 70% humidity. A warm day ahead—don't forget sunscreen!"** — 툴 결과를 정확히 소화해 자연스러운 문장으로 답했다. gemma4:e2b가 같은 테스트(4.1절)에서 완전히 실패했던 것과 정반대다.

**이 결과가 바꾸는 것.** qwen3:8b는 관문①(tools capability)에 더해 **관문③(에이전틱 추론)까지 통과하는 모델**이라는 것이 확정됐다. 유일하게 막힌 관문②는 "모델이 못한다"가 아니라 "현재 설정값이 숫자 문턱을 못 넘는다"는, 훨씬 가벼운 종류의 문제다. 4.2절 말미의 결론("이 하드웨어 위엔 교집합이 없다")은 성급했다 — 정정한다.

**컨텍스트 40960의 실체.** Qwen3-8B는 공식적으로 **32,768 토큰까지 네이티브 학습**됐고, Ollama의 기본값 40,960은 Qwen 공식 권장치(출력 32,768 + 프롬프트 8,192)를 그대로 반영한 것이다. 64K 이상으로 확장하려면 **YaRN**이라는 RoPE 스케일링 기법이 필요하며, Qwen 공식 문서는 이 방법으로 최대 131,072 토큰까지 검증했다고 명시한다. 이 사실이 중요한 사유: 앞서(4.2절) 검토했던 "Modelfile에서 `num_ctx`를 65536으로 그냥 올리는" 방법은 모델의 네이티브 학습 길이(32K)를 RoPE 스케일링 없이 무작정 초과 확장하는 것이라 품질 저하 위험이 실재하는 임시방편이었다. 반면 **YaRN은 Qwen이 공식 지원·검증한 확장 방식**이라 신뢰도가 다르다. 다만 Ollama의 GGUF 배포판이 YaRN `rope_scaling` 설정을 얼마나 매끄럽게 받아들이는지는 이번 PoC에서 검증하지 못한 남은 과제다.

**갱신된 관문 통과표:**

| 관문 | qwen3:8b | gemma4:e2b |
|---|---|---|
| ① tools capability | 통과 | 통과 |
| ② Hermes 최소 컨텍스트(64K) | 미달 (현재 설정 40960) — **YaRN으로 해소 가능성 있음** | 통과 (131K) |
| ③ 격리 테스트(툴 결과 소화) | **통과** | 실패 |
| ④ 프레임워크 연결(멀티턴 루프) | 미검증 (②를 먼저 해소해야 시도 가능) | 무의미 (③에서 이미 실패) |

즉 현재 남은 작업은 "더 똑똑한 모델을 찾는 것"이 아니라 **"이미 충분히 똑똑한 모델의 컨텍스트 설정값을 YaRN으로 안전하게 늘리는 것"**으로 좁혀졌다. 이 차이가 5.4절의 결론을 바꾼다.
...
### 5.4 개인적 결론 — 비용이 먼저, 통제는 그다음 (4.3절의 반전 이후 갱신)

배경(문서 서두)에서 밝힌 대로, 이 PoC의 원래 목표는 "비용 없이, 기밀이 새지 않게, 로컬에서 완결되는 DevOps 자동화 에이전트"였다. gemma4:e2b(4장)는 확실히 이 목표에서 탈락했다 — 에이전틱 추론 자체가 안 된다. 그런데 qwen3:8b는 4.3절의 격리 테스트 재시도로 **에이전틱 추론은 가능함이 확인**됐고, 남은 문제는 순수한 설정값(컨텍스트 40960 → 64K 이상, YaRN으로 시도 가능)으로 좁혀졌다. 즉 애초에 예상했던 "이 하드웨어엔 답이 없다"는 결론은 성급했다 — **로컬 무료 구성이 완전히 막다른 길은 아니고, 다음으로 확인할 명확한 갈래(YaRN 설정 성공 여부)가 남아 있다.**

그럼에도 개인적 판단이 크게 바뀌지는 않는다고 본다. 이유는 두 가지다.

**첫째, YaRN 확장이 성공해도 여전히 하드웨어 여유의 문제로 되돌아갈 가능성이 높다.** 32K 네이티브를 64K 이상으로 늘리면 KV 캐시가 그만큼 커지고, RAM 16GB 노트북에서 8B 모델 + 확장된 컨텍스트를 동시에 감당할 수 있을지는 별도 검증이 필요하다(다음 단계로 남김). 즉 "모델 능력"의 벽은 넘었어도 "하드웨어 용량"의 벽이 다시 나타날 수 있다.

**둘째, 설령 로컬로 64K를 확보해도, Hermes 자체가 요구하는 실사용 수준(스킬 자가생성, 150회 루프 유지, 자연스러운 대화 품질)까지 8B급 모델이 안정적으로 버텨줄지는 아직 검증되지 않았다.** 4.3절의 격리 테스트는 툴 결과 소화라는 **단일 스텝**만 확인한 것이고, 1.3절에서 짚은 "루프를 최대 150회 유지"하는 실전 내구성은 별개 검증 대상이다.

이 두 유보를 감안해도, Claude API·ChatGPT 등 유료 전환을 고려할 때 얻는 개인적 결론 자체는 유효하다고 본다: **에이전트에게 어떤 범위(디렉터리, 툴, 권한)를 주고 어떻게 통제할 것인가는, 먼저 비용(또는 그 대안인 로컬 튜닝 시간)을 감당할 방법을 정한 다음에 풀어야 할 문제**라는 순서다. API 과금은 사용량에 비례해 늘어나므로 예산 상한을 먼저 정해야 하고, 로컬로 계속 갈 경우에도 "YaRN 설정 → 하드웨어 검증 → 실전 내구성 검증"이라는 몇 단계가 통제 설계보다 먼저 온다.

이 판단이 이 문서에 남기는 사유: 6장의 보완 체크리스트를 이 순서(① YaRN으로 로컬 완결 재도전 → 안 되면 ② 비용 감당 여부 결정 → 그 다음에야 ③ 에이전트 범위·권한 설계)로 재정렬한다.
...
**① 다음 방향 결정 (최우선, 4.3절 반전으로 재갱신).** gemma4:e2b(4장)는 관문③(에이전틱 추론)에서 확실히 탈락했지만, qwen3:8b는 그 관문을 통과했고 남은 문제는 컨텍스트 설정값(40960 → 64K+)뿐임이 4.3절에서 확인됐다. 따라서 우선순위는 다음과 같이 바뀐다. (a) **먼저 YaRN 확장을 시도한다** — Qwen 공식 문서 기준 `rope_scaling` 설정으로 최대 131,072까지 검증된 방법이므로, `num_ctx`를 무작정 올리는 것보다 우선한다. Ollama의 GGUF에서 YaRN 설정이 실제로 먹히는지, 그리고 RAM 16GB에서 확장된 KV 캐시를 감당할 수 있는지가 다음 검증 대상이다. (b) YaRN 확장이 실패하거나 성능이 안 나오면, 그때 5.4절의 판단대로 **비용 감당 여부를 먼저 결정**한 뒤 OpenRouter 종량제·API 키·Nous Portal 구독 중 하나로 넘어간다. 어느 쪽으로 가든 4.1·4.3절의 검증 절차(①tools → ②컨텍스트 → ③격리테스트 → ④프레임워크 연결)를 다시 거칠 것.
...
그러나 그 차별점은 프론티어급 모델의 추론 능력 위에서 발현된다. 무료 로컬 모델 두 개를 순서대로 테스트한 결과는 이렇다 — **gemma4:e2b는 능력의 벽**에 막혔다. 툴 콜링 형식은 통과했지만 툴 결과를 소화하지 못해 에이전트 루프를 단 한 스텝도 완주하지 못했다(4장, 4.1절). **qwen3:8b는 설정의 벽**에 막혔다. Hermes 자체의 최소 컨텍스트 요구(64K)에 미달해 초기화가 거부됐지만(4.2절), 격리 테스트에서는 툴 결과를 정확히 소화해 에이전틱 추론 능력 자체는 입증했다(4.3절). 여기에 이미지 생성·본문 추출 등 일부 기능은 무료 구성에 아예 존재하지 않는다.

따라서 결론은 두 겹이다. **현재 상태 기준으로는 "프레임워크 견학"은 무료로 가능하지만 "에이전트 사용"은 불가능하다.** 다만 4.3절의 반전 덕분에 완전한 막다른 길은 아니다 — qwen3:8b + YaRN 컨텍스트 확장이라는 검증 안 된 갈래가 하나 남아 있고, 이것이 실패하면 그때는 유료 모델이 사실상 필수라는 결론이 확정된다.

이번 PoC로 얻은 개인적 결론(5.4절)은 그 갈래와 무관하게 유효하다: 유료로 전환하는 경우 API 과금은 사용량에 비례해 늘어나므로, **비용(또는 그 대안인 로컬 튜닝의 시간·하드웨어)을 먼저 선행 조건으로 확정한 뒤에야 "에이전트에게 얼마나 넓은 권한을 줄 것인가"라는 통제 설계로 넘어가는 것이 맞는 순서**라는 것. 이번 PoC는 그 순서를 몸으로 확인한 기록이다.

다음 단계: ① qwen3:8b + YaRN 확장 시도 → RAM 16GB에서 KV 캐시 감당 여부 검증(6장 ①-a) → 성공 시 Hermes 연결 후 6개 검증 시나리오 재실행 / 실패 시 ② 비용 감당 여부·예산 상한 결정(①-b) → ③ web_extract 보강 → ④ Docker 격리로 에이전트 권한 범위 설계.
