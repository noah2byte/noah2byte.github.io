...
### 4.1 원인 격리: Ollama 모델의 툴 지원 3단계 검증법
...
이 과정을 일반화하면, Ollama 로컬 모델을 에이전트 프레임워크에 붙이기 전 3단계 검증 절차가 나온다:

![모델 툴 지원 3단계 검증](/assets/img/posts/model-verification-gates.svg)

1. **`ollama show <모델>`** — capabilities에 `tools` 존재 확인 (없으면 HTTP 400으로 즉사)
2. **격리 테스트** — 위 스크립트로 tool role 메시지를 소화하는지 확인 (통과 못 하면 프레임워크에 붙여도 무의미)
3. **프레임워크 연결** — 그제서야 Hermes에 붙여 멀티턴 루프 검증

①만 믿고 세팅하면 이번처럼 ②에서 무너지는 모델에 시간을 낭비하게 된다. Qwen3 등 툴 콜링이 실전 검증된 계열은 템플릿에 tool role 렌더링이 포함되어 있어 ②를 통과한다.
...
**① 모델 교체 (최우선).** 지금 상태로는 어떤 테스트도 유의미하지 않다. (a) Ollama에서 qwen3:8b(RAM 16GB급) 또는 qwen3:14b(32GB급) 설치 후 `hermes setup model`로 교체 — **Qwen3를 첫 후보로 꼽는 사유: Ollama 템플릿에 tool role 렌더링이 포함되어 4.1의 격리 테스트를 통과하는 계열이고, 로컬 모델 중 에이전틱 툴 콜링이 실전에서 가장 안정적이라는 평가를 받기 때문.** (b) 로컬 하드웨어가 부족하면 OpenRouter 무료 티어로 원격 테스트, (c) 제대로 쓸 거면 Anthropic API 키 또는 Nous Portal. 어떤 모델이든 Hermes에 붙이기 전 4.1의 3단계 검증을 먼저 거칠 것.
...
**⑥ 보안 격리 (DevOps 시각).** local 백엔드는 에이전트가 내 계정 권한으로 임의 셸을 실행한다. 소형 모델일수록 오동작 확률이 높으므로 본격 실험 전 Docker 백엔드로 격리 권장. 파일 변경은 체크포인트/`/rollback`으로 되돌릴 수 있지만 **셸 명령의 부수효과는 롤백되지 않는다.**
...
Hermes Agent는 설치 UX(자동 의존성 설치, 단계별 무료/유료 표기, config 자동 백업), 오류 리포팅, 프로바이더 추상화 등 프레임워크 완성도가 높고, 스킬 자가생성·세션 간 기억이라는 뚜렷한 차별점이 있다. 무료 구성만으로 설치부터 툴 파이프라인 관찰까지의 PoC는 충분히 가능하다.

그러나 그 차별점은 전부 프론티어급 모델의 추론 능력 위에서만 발현된다. 이번 PoC에서 무료 로컬 소형 모델은 툴 콜링 형식은 통과했지만 에이전트 루프를 단 한 스텝도 완주하지 못했고, 이미지 생성·본문 추출 등 일부 기능은 무료 구성에 아예 존재하지 않는다. **"프레임워크 견학"은 무료로 가능하지만 "에이전트 사용"에는 Claude 등 유료 모델 구독 또는 API 과금이 사실상 필수**라는 것이 결론이다.

다음 단계: ① 모델 교체 → ② web_extract 보강 → ③ Docker 격리 → 동일한 6개 검증 시나리오 재실행 후 비교.
